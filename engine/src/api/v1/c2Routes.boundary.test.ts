...
describe('createSphereRoutes boundary hardening', () => {
  ...
  let conductor: {
    ...
    verifyThreadLedger: ReturnType<typeof vi.fn>;
  ...
  beforeAll(async () => {
    ...
    conductor = {
      ...
      verifyThreadLedger: vi.fn(async () => null),
      getConductorSignatureProfile: vi.fn(() => ({
        mode: 'hmac_sha256_internal',
        algorithms: ['hmac_sha256'],
        ed25519KeyId: null
      })),
      getConductorSignatureVerificationPolicy: vi.fn(() => ({
        requireV2: false,
        activationAt: null,
        graceDays: 0
      })),
  ...
  beforeEach(() => {
    ...
    conductor.verifyThreadLedger.mockReset();
    ...
    conductor.getConductorSignatureProfile.mockReturnValue({
      mode: 'hmac_sha256_internal',
      algorithms: ['hmac_sha256'],
      ed25519KeyId: null
    });
    conductor.getConductorSignatureVerificationPolicy.mockReturnValue({
      requireV2: false,
      activationAt: null,
      graceDays: 0
    });
  ...
  it('publishes hardened capabilities contract', async () => {
    ...
    expect(response.body.protocol?.cycleEventTaxonomy?.initialEventTypes).toEqual(['seat_taken']);
    expect(response.body.protocol?.ledgerVerification).toEqual({
    ...
    expect(response.body.signatures?.conductorSigningMode).toBeDefined();
    expect(Array.isArray(response.body.signatures?.conductorAlgorithms)).toBe(true);
    expect(response.body.signatures?.conductorEd25519KeyId).toBeNull();
    expect(response.body.signatures?.conductorV2Verification).toEqual({
      requireV2: false,
      activationAt: null,
      graceDays: 0
    });
  ...
  it('publishes dual-sign conductor capability metadata when configured', async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body.signatures?.conductorSigningMode).toBe('dual_hmac_sha256_plus_ed25519');
    expect(response.body.signatures?.conductorAlgorithms).toEqual(['hmac_sha256', 'ed25519']);
    expect(response.body.signatures?.conductorEd25519KeyId).toBe('conductor-key-2026-03');
  });
  ...
  it('returns canonical cycle-state snapshot for a thread', async () => {
    conductor.getThread.mockResolvedValueOnce({
      threadId: '11111111-1111-4111-8111-111111111111',
      missionId: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
      createdAt: '2026-01-01T00:00:00.000Z',
    ...
    const response = await request
      .get('/api/v1/sphere/threads/11111111-1111-4111-8111-111111111111/cycle-state')
      .set('authorization', `Bearer ${token}`);

    expect(response.status).toBe(200);
    expect(response.body.threadState).toBe('ACTIVE');
    expect(response.body.phase).toBe('synthesis_returned');
    expect(response.body.expectedNextEventTypes).toEqual(['lens_upgraded']);
    expect(response.body.initialEventTypes).toEqual(['seat_taken']);
    expect(response.body.cycleStarted).toBe(true);
    expect(response.body.completedRounds).toBe(0);
    expect(response.body.eventCounts).toEqual({
      seat_taken: 1,
      perspective_submitted: 1,
      synthesis_returned: 1,
      lens_upgraded: 0
    });
    expect(response.body.lastEvent).toMatchObject({
  ...
  it('returns 404 from cycle-state when thread is missing', async () => {
  ...
  it('returns ledger verification report for a thread', async () => {
    conductor.verifyThreadLedger.mockResolvedValueOnce({
      threadId: '11111111-1111-4111-8111-111111111111',
      verified: true,
      checkedEntries: 3,
      latestSequence: 3,
    ...
    const response = await request
      .get('/api/v1/sphere/threads/11111111-1111-4111-8111-111111111111/verify-ledger')
      .set('authorization', `Bearer ${token}`);

    expect(response.status).toBe(200);
    expect(response.body.verified).toBe(true);
    expect(response.body.checkedEntries).toBe(3);
    expect(response.body.latestSequence).toBe(3);
    expect(response.body.latestEntryHash).toBe('abc123');
    expect(Array.isArray(response.body.issues)).toBe(true);
    expect(conductor.verifyThreadLedger).toHaveBeenCalledWith(
      '11111111-1111-4111-8111-111111111111'
    );
  });

  it('returns ledger verification signature issue codes when present', async () => {
    conductor.verifyThreadLedger.mockResolvedValueOnce({
      threadId: '11111111-1111-4111-8111-111111111111',
      verified: false,
      checkedEntries: 2,
      ...
      issues: [
        {
          ...
          message: 'Missing conductorSignatureV2 at sequence 2.',
          sequence: 2
        }
      ]
    });
  ...
  it('returns 404 from verify-ledger when thread is missing', async () => {
    conductor.verifyThreadLedger.mockResolvedValueOnce(null);
  ...
  it('returns conductor key registry list', async () => {
    conductor.listConductorKeys.mockResolvedValueOnce([
      {
        keyId: 'conductor-key-2026-03',
        publicKey: '-----BEGIN PUBLIC KEY-----fake-----END PUBLIC KEY-----',
        status: 'ACTIVE',
  ...
  it('rotates conductor key through route', async () => {
    conductor.rotateConductorKey.mockResolvedValueOnce({
      key: {
        keyId: 'conductor-key-2026-04',
        publicKey: '-----BEGIN PUBLIC KEY-----rotated-----END PUBLIC KEY-----',
        status: 'ACTIVE',
        activationDate: '2026-03-11T00:00:00.000Z',
        retirementDate: null,
        verificationGraceDays: 0,
      ...
      privateKeyPem: '-----BEGIN PRIVATE KEY-----rotated-----END PRIVATE KEY-----'
    });
    ...
    expect(response.status).toBe(201);
    expect(response.body.rotatedKey?.keyId).toBe('conductor-key-2026-04');
