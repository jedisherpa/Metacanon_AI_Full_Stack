...
| # | Area | Decision |
| :--- | :--- | :--- |
| 2 | **Compute Stack:** What is the compute architecture? | The system will implement a pluggable compute stack with seven providers: `Qwen` (default local), `Ollama`, `Morpheus`, `OpenAI`, `Anthropic`, `Moonshot Kimi`, and `Grok`. The architecture must support a global default provider and per-deliberation overrides. |
