...
use crate::action_validator::WillVectorActionValidator;
...
use crate::prism::{DefaultPrism, Prism, PrismError, PrismSynthesisRequest};
...
use crate::torus::{ActionValidator as TorusActionValidator, DefaultActionValidator};
use crate::workflow::{
...
fn build_will_vector_validator(state: &UiState) -> WillVectorActionValidator {
    ...
    // Runtime default is policy-blocklist enforcement. Strict directive alignment is opt-in.
...
pub fn submit_deliberation(
    ...
    let mut preflight_request = GenerateRequest::new(query.clone());
    if let Some(provider_id) = normalized_override.clone() {
        preflight_request = preflight_request.with_provider_override(provider_id);
    }
    if let Err(validation_error) = DefaultActionValidator.validate_action(&preflight_request) {
    ...
    if let Err(validation_error) = will_validator.validate_generate_request(&preflight_request) {
        return Err(UiCommandError::DeliberationFailed {
            requested_provider: requested_provider_id.clone(),
            attempts: vec![ProviderAttemptError {
                provider_id: requested_provider_id,
    ...
    let mut prism_request = PrismSynthesisRequest::new(query.clone(), vec![query]);
    prism_request.provider_override = normalized_override.clone();
    prism_request.metadata.insert(
        "runtime_path".to_string(),
        "ui.submit_deliberation.prism".to_string(),
    );

    match prism.synthesize(&state.compute_router, prism_request) {
        Ok(output) => {
            let used_fallback = output.provider_id != requested_provider_id;
        ...
        Err(PrismError::EmptyQuery | PrismError::NoInputs) => {
            Err(UiCommandError::DeliberationFailed {
                requested_provider: requested_provider_id.clone(),
                attempts: vec![ProviderAttemptError {
                    provider_id: requested_provider_id,
                    error_kind: compute_error_kind_label(&ComputeErrorKind::InvalidRequest)
                        .to_string(),
                    message: "prism synthesis request is incomplete".to_string(),
                }],
            })
        }
    }
...
pub fn send_agent_message(
    ...
    let mut state = runtime.lock_state()?;
    let channel = format!("agent:{communication_platform:?}");
    let normalized_message =
        validate_outbound_message_against_constitution(&state, &channel, &message)?;
...
pub fn send_sub_sphere_prism_message(
    ...
    let communication_platform = parse_communication_platform(&platform)?;
    let mut state = runtime.lock_state()?;
    let channel = format!("sub_sphere_prism:{communication_platform:?}");
    let normalized_message =
        validate_outbound_message_against_constitution(&state, &channel, &message)?;
