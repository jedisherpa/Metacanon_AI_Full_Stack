...
describe("getArrivalContext", () => {
  it("parses inbound realm parameters and passthrough context", () => {
    const arrival = getArrivalContext(
      "?from=castle-paul&uuid=abc123&held_artifact=palace-of-perfection&quest=return-home"
    );
    ...
    expect(arrival).toEqual({
      ...
      incomingUuid: "abc123",
      heldArtifact: "palace-of-perfection",
      quest: "return-home",
    });
  });
...
describe("buildPortalUrl", () => {
  it("preserves existing params while appending Diana realm identity", () => {
    const url = new URL(
    ...
    expect(url.searchParams.get("from")).toBe("castle-diana");
...
describe("deriveSovereignBadge", () => {
  it("awards Shadowbreaker Initiate after five distinct artifacts", () => {
    ...
    expect(badge?.id).toBe("shadowbreaker-initiate");
  });

  it("awards Luminous Wayfinder after three distinct portal exits", () => {
    const badge = deriveSovereignBadge([
      {
    ...
    expect(badge?.id).toBe("luminous-wayfinder");
  });

  it("awards Throne Claimed after declaration activity", () => {
    const badge = deriveSovereignBadge([
      { event_type: "ddos_sign", artifact_id: null, metadata: null, portal_id: null },
    ]);

    expect(badge?.id).toBe("throne-claimed");
  });
});
