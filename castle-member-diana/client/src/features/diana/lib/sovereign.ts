...
const UUID_KEY = "diana:sovereign:uuid";
const QUEUE_KEY = "diana:sovereign:queue";
const LOCAL_LEDGER_KEY = "diana:sovereign:local-ledger";
const SEEN_BADGES_KEY = "diana:sovereign:seen-badges";
const SESSION_EVENT_KEY = "diana:sovereign:session-events";
...
function mapRealmName(realm: string | null) {
  ...
  return DIANA_MEMBER_CONFIG.realmNames[realm] ?? realm.replace(/[-_]/g, " ");
}
...
function buildLedgerRow(
  ...
  return {
    uuid,
    realm_id: import.meta.env.VITE_REALM_ID ?? DIANA_MEMBER_CONFIG.realmId,
...
export function deriveSovereignBadge(rows: BadgeLedgerRow[]): SovereignBadge | null {
  ...
  if (artifactCount >= 5) {
    return {
      id: "shadowbreaker-initiate",
      title: "Shadowbreaker Initiate",
      description:
        "All five artifacts have been opened. The realm recognizes a complete first passage through Diana's thresholds."
    };
  }

  if (portalRealms.size >= 3) {
    return {
      id: "luminous-wayfinder",
      title: "Luminous Wayfinder",
      description:
        "Three outbound portals have been crossed. The ecosystem now knows this traveler by motion, not only by intention."
    };
  }
  ...
  if (hasAscended) {
    return {
      ...
      description:
        "A declaration action has been completed. The visitor has stepped from witness into public alignment."
    };
  }
...
export function buildPortalUrl(baseUrl: string, uuid: string, destinationRealm: string) {
  ...
  url.searchParams.set("from", DIANA_MEMBER_CONFIG.realmId);
...
export async function fetchWallEntries(): Promise<WallEntry[]> {
  const fallback = DIANA_MEMBER_CONFIG.wallSeeds.map((label, index) => ({
    id: `seed-${index}`,
    label,
