...
const UUID_KEY = "anna:sovereign:uuid";
const QUEUE_KEY = "anna:sovereign:queue";
const LOCAL_LEDGER_KEY = "anna:sovereign:local-ledger";
const SEEN_BADGES_KEY = "anna:sovereign:seen-badges";
const SESSION_EVENT_KEY = "anna:sovereign:session-events";
...
function mapRealmName(realm: string | null) {
  ...
  return ANNA_MEMBER_CONFIG.realmNames[realm] ?? realm.replace(/[-_]/g, " ");
}
...
function buildLedgerRow(
  ...
  return {
    uuid,
    realm_id: import.meta.env.VITE_REALM_ID ?? ANNA_MEMBER_CONFIG.realmId,
...
export function buildPortalUrl(baseUrl: string, uuid: string, destinationRealm: string) {
  ...
  url.searchParams.set("from", ANNA_MEMBER_CONFIG.realmId);
...
export async function fetchWallEntries(): Promise<WallEntry[]> {
  const fallback = ANNA_MEMBER_CONFIG.wallSeeds.map((label, index) => ({
    id: `seed-${index}`,
    label,
