...
impl ActionValidator for DefaultActionValidator {
    fn validate_action(
        ...
        if let Some(reason) = request.metadata.get("blocked_by_will_vector") {
            return Err(ValidationError::BlockedTerm(reason.clone()));
        }
...
impl WillVectorActionValidator {
    ...
    pub fn validate_generate_request(
        ...
        self.validate_action_request(&ActionRequest::llm_call(request.prompt.clone()))
    }
    ...
    pub fn validate_tool_invocation(
        ...
        self.validate_action_request(&ActionRequest::tool_invocation(tool_id, arguments))
    }
    ...
    pub fn validate_outbound_message(
        ...
        self.validate_action_request(&ActionRequest::external_message(channel, content))
    }
...
mod tests {
    ...
    #[test]
    fn default_validator_rejects_empty_prompt() {
        let result = DefaultActionValidator.validate_action(&GenerateRequest::new("   "));
    ...
    fn validate_action_rejects_empty_content() {
        ...
        let result = validator.validate_action_request(&action);
        assert!(matches!(result, Err(ValidationError::EmptyContent)));
    }
