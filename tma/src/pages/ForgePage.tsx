...
const DEFAULT_CYCLE_INITIAL_EVENTS: SphereCycleEventType[] = ['seat_taken'];
...
function fallbackNextCycleEvents(eventType: SphereCycleEventType): SphereCycleEventType[] {
  switch (eventType) {
    case 'seat_taken':
      return ['perspective_submitted'];
    case 'perspective_submitted':
      return ['synthesis_returned'];
    case 'synthesis_returned':
      return ['lens_upgraded'];
    case 'lens_upgraded':
      return ['seat_taken', 'perspective_submitted'];
    default:
      return [...DEFAULT_CYCLE_INITIAL_EVENTS];
  }
}
...
export default function ForgePage({ profile }: Props) {
  ...
  const [cycleExpectedNextEventTypes, setCycleExpectedNextEventTypes] = useState<SphereCycleEventType[]>(
    [...DEFAULT_CYCLE_INITIAL_EVENTS]
  );
  const [cycleCompletedRounds, setCycleCompletedRounds] = useState(0);
  const [cycleThreadState, setCycleThreadState] = useState<string | null>(null);
  ...
  const cycleEventsEnabled = sphereCapabilities
    ...
    : false;
  const cycleStateEnabled = sphereCapabilities
    ? cycleFeatures?.cycleState !== false
    : false;
  ...
  const expectedCycleEventSet = useMemo(
    () => new Set<SphereCycleEventType>(cycleExpectedNextEventTypes),
    [cycleExpectedNextEventTypes]
  );
  const canTakeSeat =
    cycleEventsEnabled &&
    !writesBlocked &&
    !submittingCycle &&
    expectedCycleEventSet.has('seat_taken');
  const canSubmitPerspective =
    cycleEventsEnabled &&
    !writesBlocked &&
    !submittingCycle &&
    expectedCycleEventSet.has('perspective_submitted') &&
    Boolean(cyclePerspective.trim());
  const canRecordLensUpgrade =
    cycleEventsEnabled &&
    !writesBlocked &&
    !submittingCycle &&
    expectedCycleEventSet.has('lens_upgraded');
  ...
  function applyCycleStateSnapshot(snapshot: SphereCycleStateResponse): void {
    setCyclePhase(snapshot.phase);

    const expected = Array.isArray(snapshot.expectedNextEventTypes)
      ? snapshot.expectedNextEventTypes.filter(Boolean)
      : [];
    if (expected.length > 0) {
      setCycleExpectedNextEventTypes(expected);
    } else if (Array.isArray(snapshot.initialEventTypes) && snapshot.initialEventTypes.length > 0) {
      setCycleExpectedNextEventTypes(snapshot.initialEventTypes);
    } else {
      setCycleExpectedNextEventTypes([...DEFAULT_CYCLE_INITIAL_EVENTS]);
    }

    setCycleCompletedRounds(snapshot.completedRounds ?? 0);
    setCycleThreadState(snapshot.threadState ?? null);
  }
  ...
  async function syncReplay(threadId: string): Promise<void> {
    if (!replayEnabled && !cycleFeatures?.threadAcks && !cycleStateEnabled) {
      return;
    }
    ...
    try {
      ...
      if (cycleStateEnabled) {
        tasks.push(
          api.getSphereCycleState(threadId).then((response) => {
            applyCycleStateSnapshot(response);
          })
        );
      }
  ...
  async function writeCycleEvent(eventType: SphereCycleEventType, payload: Record<string, unknown>) {
    ...
    if (!cycleThreadId) {
      ...
      setCyclePhase(null);
      setCycleExpectedNextEventTypes([...DEFAULT_CYCLE_INITIAL_EVENTS]);
      setCycleCompletedRounds(0);
      setCycleThreadState(null);
      resetStreamState();
    }
    ...
    if (!cycleStateEnabled) {
      setCycleExpectedNextEventTypes(fallbackNextCycleEvents(eventType));
    }
        ...
        {!loading && activeTab === 'cycle' && (
          <div className="p-4 space-y-5">
            ...
            <div className="text-xs text-white/35 font-mono space-y-2">
              ...
              <p>
                Next expected:{' '}
                {cycleExpectedNextEventTypes.length > 0
                  ? cycleExpectedNextEventTypes.map((eventType) => cycleEventLabel(eventType)).join(', ')
                  : 'none'}
              </p>
              <p>Completed rounds: {cycleCompletedRounds}</p>
              <p>Thread state: {cycleThreadState ?? sphereStatus?.systemState ?? 'unknown'}</p>
